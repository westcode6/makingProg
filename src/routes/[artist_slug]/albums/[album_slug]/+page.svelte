...
<main class="w-full min-h-[100dvh] bg-dark text-white -mt-16">
	...
	<section class="h-auto xl:max-w-7xl mx-auto relative grid grid-cols-12">
		...
		>
			<article class="mb-12">
				<p class="text-sm text-blue-200">ALBUM</p>
				<p class="text-5xl extrabold mb-3">{album?.title}</p>
				<p class="text-sm text-neutral-300 mb-2">Total songs: {album?.songs?.length}</p>
				<p class="text-sm mb-7 text-gray-100">
					{album?.description}
				</p>

				<button class="btn btn-warning"
					>Buy album ({formatCurrency(album?.amount, album?.currency)})</button
				>
			</article>
					...
					{:then data}
						{#each data.data as single, index}
							<li class="flex items-center justify-between gap-4 border-b border-b-gray-900 py-4">
								...
								<a href="/singles/{single?.slug}" class="btn btn-sm mr-2">View</a>
							</li>
						{/each}
					{/await}
				</ul>
		...
		<picture
			...
			style="background: linear-gradient(to left, rgba(0,0,0,0) 25%, 75%, #1A1A23), linear-gradient(to bottom, rgba(0,0,0,0) 25%, 75%, #1A1A23), url('{album
				?.cover_art?.url}'); background-position: top; background-size: cover;"
		/>
	</section>
</main>
